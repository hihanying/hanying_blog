...
import top.hihanying.hyblog.commons.entity.FileSort;
...
import top.hihanying.hyblog.picture.mapper.FileMapper;
import top.hihanying.hyblog.picture.service.*;
import top.hihanying.hyblog.picture.util.AboutFileUtil;
import top.hihanying.hyblog.picture.util.FeignUtil;
import top.hihanying.hyblog.utils.FileUtils;
...
import top.hihanying.hyblog.base.enums.EFilePriority;
import top.hihanying.hyblog.base.enums.EOpenStatus;
import top.hihanying.hyblog.base.enums.EStatus;
import top.hihanying.hyblog.base.exception.exceptionType.InsertException;
...
public class FileServiceImpl extends SuperServiceImpl<FileMapper, File> implements FileService {
    ...
    public String getPicture(String fileIds, String code) {
        ...
        if (StringUtils.isEmpty(fileIds)) {
            ...
            QueryWrapper<top.hihanying.hyblog.commons.entity.File> queryWrapper = new QueryWrapper<>();
            queryWrapper.in(SQLConf.UID, changeStringToString);
            List<top.hihanying.hyblog.commons.entity.File> fileList = fileService.list(queryWrapper);
            if (fileList.size() > 0) {
                for (top.hihanying.hyblog.commons.entity.File file : fileList) {
                    if (file != null) {
    ...
    public String batchUploadFile(HttpServletRequest request, List<MultipartFile> filedatas, SystemConfig systemConfig) {
        ...
        if (SysConf.PICTURE.equals(source)) {
            // 当从vue-hanying-web网站过来的，直接从参数中获取
    ...
    public String uploadPictureByUrl(FileVO fileVO) {
        ...
        List<top.hihanying.hyblog.commons.entity.File> lists = new ArrayList<>();
        //文件上传
        if (urlList != null && urlList.size() > 0) {
            for (String itemUrl : urlList) {
                ...
                top.hihanying.hyblog.commons.entity.File file = new top.hihanying.hyblog.commons.entity.File();
                file.setCreateTime(new Date(System.currentTimeMillis()));
                file.setFileSortUid(fileSort.getUid());
